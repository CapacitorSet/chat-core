...
Server.prototype = {
    ...
    sequelize.query('SELECT * FROM ConvoUserPairs').success(function(result){

        /*
        result = [ { id: 1, convo: '#a', user: 'afilini', level: 2 },
  { id: 2, convo: '#b', user: 'afilini', level: 2 } ]
        
        server.channels.perm = { #a: { afilini: 2 }, #b: { afilini:2 }}
        */

        result.forEach(function(item) {

            /* Adds an element to the item "server.channels.perm[item.convo]" */
            var temp = server.channels.perm[item.convo];
            if (temp === undefined) { temp = {}; }
            temp[item.user] = item.level;
            server.channels.perm[item.convo] = temp;
        })
    });
    ...
    respond: function(data, client) {
        ...
        if (this.validCommand(message.command)) {
            /* Credo che questo blocco di codice sia inutile, ma non sono sicuro.
