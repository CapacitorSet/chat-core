var net = require('net'),
    ...
    tehChannel = require('./channel').tehChannel,
...
Server.prototype = {
    ...
    sequelize.query('SELECT DISTINCT convo FROM ConvoUserPairs').success(function(result){
        result.forEach(function(item) {
            // Generates an appropriate tehChannel
            server.channels[item.convo] = new tehChannel(item.convo, server, function() {
                console.log('done loading', item.convo);
            });
        });
    });
