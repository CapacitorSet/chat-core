...
Server.prototype = {
    ...
    startTimeoutHandler: function() {
        ...
        this.timeoutHandler = setInterval(function() {
            ...
            // todo: for some reason, pings aren't sent sometimes
            self.users.forEach(function(user) {
                if (!(user.hasPonged)) {
                    var index = self.clients[user.username].indexOf(user.clientId);
                    self.clients[user.username].splice(index, 1);
                    colog.info(colog.bold('[' + colog.yellow(user.username) + '] User timed out: New list clients connected: '+JSON.stringify(self.clients[user.username])));
    ...
    start: function(callback) {
        ...
        function handleStream(stream) {
            try {
                ...
                client.object.deviceId = deviceId;
