...
Commands.prototype = {
    ...
    USER: function(user, username, hostname, servername, realname) {
        ...
        fs.readFile('profiles/'+md5(username), 'utf8', function(err, data) {
            ...
            self.users.register(user, username, hostname, servername, realname);
        });
    },
    ...
    TIME: function(user, server) {
        user.send(this.server.host, irc.reply.time, user.nick, this.server.config.hostname, ':' + (new Date().getTime()));
    },
