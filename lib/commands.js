    ...
    FETCH: function(user, timestamp) {
        sequelize.models.Message.findAll({
            ...
            items.forEach(function(message) {
                user.send(JSON.stringify(message.dataValues));
            })
        })
    },
...
//      user.mode = 'light';
        ...
        sequelize.models.User.find({
            ...
            ircd.compareHash(user.password, item.selectedValues.salt, item.selectedValues.password, function(res) {
                if (res) {
                    ...
                    var clientId = md5(String(new Date().getTime()));
                    user.clietnId = clientId;

                    if (!self.clients[user.username]) {
                        self.clients[user.username] = [];
                    }
                    self.clients[user.username].push(clientId);
                    user.send(JSON.stringify(self.clients[user.username]));
                } else {
                    user.send(self.host, irc.errors.passwordWrong, username, ':Password incorrect');
                    user.quit();
                    return;
    ...
    QUIT: function(user) {
        ...
        var index = this.server.clients[user.username].indexOf(user.clietnId);
        this.server.clients[user.username].splice(index, 1);
        colog.info(colog.bold('[' + colog.yellow(user.username) + '] New list clients connected: '+JSON.stringify(this.server.clients[user.username])));
        delete user;
    },
