...
Commands.prototype = {
    ...
    /*
     *  LW: Switch to lightweight mode (enable broadcasted messages from other devices with you account, broadcasted welcome from channels...)
     *
     *  Why don't we merge fetch and lw? The command fetch should be called right after you have logged, and, since the client knows the existence
     *  of the command fetch (not included in the irc standard), we can automatically switch to lightweight mode.
     */
    LW: function(client){
        client.light = true;
        colog.info(colog.bold('[' + colog.blue(client.username + '@' + client.deviceId) + '] Switched to LW Mode'));
    },
    ...
    PRIVMSG: function(client, target) {
        ...
        if (!target || target.length === 0) {
            ...
            if (recipient) {
                user.sendSpecial(user.mask, 'PRIVMSG', recipient.username, ':' + message);
