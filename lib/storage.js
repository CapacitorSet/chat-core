...
ChannelDatabase.prototype = {
    ...
    join: function(user, channelName) {
        ...
        if (!channel) {
            // se il canale non è in memoria, viene creato
        ...
        if (channel.users.length === 0) {
            // Se è la prima persona a entrare nel canale, oppalo. (WTF, right?)
            // Va messo fuori dall'if perchè si può applicare sia ai canali nuovi, sia a quelli esistenti ma che non hanno più utenti
            user.op(channel);
        } else {
            // controlla se l'utente può entrare nel canale
            if (!channel.onInviteList(user)) {
